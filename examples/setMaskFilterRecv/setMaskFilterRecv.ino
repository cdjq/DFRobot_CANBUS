...
void setup()
{
    ...
    do {
        ...
        CAN.initMask(CAN.MCP_RXM0, 0, 0x3ff);                         // there are 2 mask in mcp2515, you need to set both of them
        CAN.initMask(CAN.MCP_RXM1, 0, 0x3ff);
        /*
         ...
         */
        CAN.initFilter(CAN.MCP_RXF0, 0, 0x04);                         // filter 0 for id = 0x04
        CAN.initFilter(CAN.MCP_RXF1, 0, 0x05);                         // filter 1 for id = 0x05
        // CAN.init_Filter(MCP_RXF2, 0, 0x06);                         // filter 2 for id = 0x06
        CAN.initFilter(CAN.MCP_RXF3, 0, 0x07);                         // filter 3 for id = 0x07
        CAN.initFilter(CAN.MCP_RXF4, 0, 0x08);                         // filter 4 for id = 0x08
        CAN.initFilter(CAN.MCP_RXF5, 0, 0x09);                         // filter 5 for id = 0x09 
